...
class StudentReservationController extends Controller
{
    ...
    public function getReservationsByUserId($user_id)
    {
        try {
            ...
            foreach ($userReservations as $userReservation) {
                ...
                $bookDetail->image_url = $bookDetail->image_url ? config('app.url') . Storage::url($bookDetail->image_url) : null;
    ...
    public function getBorrowedById($id)
    {
        try {
            ...
            $borrowedMaterial->material->image_url = $borrowedMaterial->material->image_url ? config('app.url') . Storage::url($borrowedMaterial->material->image_url) : null;
    ...
    public function viewReservationById($id)
    {
        ...
        $reservation->book->image_url = $reservation->book->image_url ? config('app.url') . Storage::url($reservation->book->image_url) : null;

        Log::info('Reservation details: ' . json_encode($reservation));
        if ($reservation) {
